...
class AppViewModel: ObservableObject {
    ...
    init() {
        ...
        self.bbcSounds = BBCSoundsService()
    ...
    func playProgramme(_ programme: Programme) async {
        ...
        errorMessage = nil
        do {
            let url = try await bbcSounds.getStreamURL(pid: programme.id)
            player.play(url: url, programme: programme)
        } catch {
    ...
    func autoPlay(pid: String) async {
        ...
        if pid.hasPrefix("m") || pid.hasPrefix("p") || pid.count == 8 {
            do {
                ...
                let url = try await bbcSounds.getStreamURL(pid: pid)
                player.play(url: url, programme: programme)
            } catch {
                ...
                if let url = try? await bbcSounds.getStreamURL(pid: pid) {
                     let dummyProgramme = Programme(id: pid, index: 0, name: "Auto-Play Stream", channel: "BBC", duration: nil, description: pid, firstBroadcast: nil, artworkURL: nil)
